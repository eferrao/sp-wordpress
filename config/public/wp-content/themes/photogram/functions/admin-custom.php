...
.colabs_metaboxes_table td .ui-datepicker-trigger{margin-left: 5px;}
</style>
<?php
}
...
function colabs_custom_enqueue($hook) {
    if ('post.php' == $hook OR 'post-new.php' == $hook OR 'page-new.php' == $hook OR 'page.php'== $hook) {
        ...
        wp_enqueue_style( 'jquery-ui-datepicker-css', get_template_directory_uri() . '/functions/css/jquery-ui-datepicker.css' );
    }
}
...
function colabs_custom_meta_generator($post,$colabs_metaboxes){
  $output = '';
  ...
  foreach ($colabs_metaboxes as $colabs_metabox) { // On Save.. this gets looped in the header response and saves the values submitted
    ...
    OR 'images' == $colabs_metabox['type'] ) // Normal Type Things...
    {
      ...
      if ( isset( $_POST[$var] ) ) {
          ...
          if(get_post_meta( $post_id, $var ) == "") {

              add_post_meta( $post_id, $var, $posted_value, true );

          }
          // Otherwise, if it's different, update the post meta.
          elseif( $posted_value != get_post_meta( $post_id, $var, true ) ) {

              update_post_meta( $post_id, $var, $posted_value );

          }
          // Otherwise, if no value is set, delete the post meta.
          elseif($posted_value == "") {

              delete_post_meta( $post_id, $var, get_post_meta( $post_id, $var, true ) );
          ...
          update_post_meta( $post_id, $var, 'false' );

      } else {

          delete_post_meta( $post_id, $var, $current_value ); // Deletes check boxes OR no $_POST
      ...
      $_FILES['attachement_'.$id]['name'] = preg_replace( '/[^a-zA-Z0-9._\-]/', '', $_FILES['attachement_'.$id]['name']);
             ...
             update_post_meta( $post_id, $id, $uploaded_file['url'] );
          ...
          update_post_meta($post_id, $id, $posted_value);
