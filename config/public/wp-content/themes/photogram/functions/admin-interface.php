...
function colabsthemes_add_admin() {
  ...
  if ( isset( $_REQUEST['page'] ) ) {
      ...
      $_page = '';
    $_page = sanitize_key( strtolower( trim( strip_tags( $_REQUEST['page'] ) ) ) );
    // Sanitize action being requested.
      $_action = '';
    if ( isset( $_REQUEST['colabs_save'] ) ) {
      $_action = sanitize_key( strtolower( trim( strip_tags( $_REQUEST['colabs_save'] ) ) ) );
    } // End IF Statement
  ...
  if(get_option( 'framework_colabs_backend_icon')) { 
    $icon = get_option( 'framework_colabs_backend_icon' ); 
  } else { 
  ...
  if(function_exists( 'add_object_page')){
    add_object_page ( __('Theme Options', 'colabsthemes'), $themename, 'manage_options','colabsthemes','colabsthemes_options_page', $icon);
  }else{
    add_theme_page ( __('Theme Options', 'colabsthemes'), $themename, 'manage_options','colabsthemes','colabsthemes_options_page', $icon);
  }
  ...
  if($super_user == $current_user_id || empty($super_user)) {
    $colabsthemepage = add_submenu_page( 'colabsthemes', 'ColorLabs Update', 'Updates', 'manage_options', 'colabsthemes_framework_update', 'colabsthemes_framework_update_page' );
  }
...
function colabs_admin_styles() {
  ...
  wp_enqueue_style( 'colabs-admin-style', get_template_directory_uri() . '/functions/css/admin-style.css' );
