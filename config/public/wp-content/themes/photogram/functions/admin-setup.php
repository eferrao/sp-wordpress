...
add_action( 'wp_head', 'colabsthemes_wp_head', 20 );

/*-----------------------------------------------------------------------------------*/
/* Output HEAD - Default Style */
/*-----------------------------------------------------------------------------------*/

add_action( 'wp_enqueue_scripts', 'colabsthemes_add_style_head', 20 );

if ( ! function_exists( 'colabsthemes_add_style_head' ) ) {
	function colabsthemes_add_style_head() {
    // Output alternative stylesheet
...
if (!function_exists( 'colabs_output_alt_stylesheet')) {
	function colabs_output_alt_stylesheet() {
		...
		if ($style != '') {
      wp_enqueue_style( 'colabs-skin-style', get_template_directory_uri() .'/styles/'. $style );
		} else {
        ...
        wp_enqueue_style( 'colabs-skin-style', get_template_directory_uri() .'/styles/'. $style );
				...
				if (file_exists(get_template_directory() .'/styles/default.css')):
          wp_enqueue_style( 'colabs-skin-style', get_template_directory_uri() .'/styles/default.css' );
				endif;
			}
		}
...
if (!function_exists( 'colabs_output_custom_css')) {
    ...
    wp_enqueue_style( 'colabs-custom-css', trailingslashit( $upload_dir['baseurl'] ).strtolower(COLABS_THEME_NAME).'-custom/custom.css');
...
add_action( 'wp_enqueue_scripts', 'colabs_comment_reply');
?>