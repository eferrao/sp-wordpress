	...
	?>
		...
		<div class="gallery-metabox">
			...
			<select name="meta_style_gallery" id="meta_style_gallery" class="select-gallery">
				...
				<?php if(ColabsInstagram::getInstance()->getAccessToken()):?>
			...
			<div id="instagram_gallery_block" style="display:none">
				<p><strong><?php _e( 'Type:', 'colabsthemes' ); ?></strong></p>
				<select id="colabs_type_instagram" name="colabs_type_instagram" class="colabs-input">
					<option <?php if(get_post_meta($post->ID,'colabs_type_instagram',true)=='')echo 'selected="selected"';?> value=""><?php _e( 'Popular', 'colabsthemes' ); ?></option>
					<option <?php if(get_post_meta($post->ID,'colabs_type_instagram',true)=='self')echo 'selected="selected"';?> value="self"><?php _e( 'Self', 'colabsthemes' ); ?></option>
					<option <?php if(get_post_meta($post->ID,'colabs_type_instagram',true)=='myfeed')echo 'selected="selected"';?> value="myfeed"><?php _e( 'Feed', 'colabsthemes' ); ?></option>
				</select>
				<p>
					<strong><?php _e( 'Count Instagram:', 'colabsthemes' ); ?></strong>
				</p>
				<p>	
					<input type="text" value="<?php echo get_post_meta($post->ID,'colabs_piccount_instagram',true);?>" id="count_instagram" size="4" name="colabs_piccount_instagram"><br/><?php _e( 'Count of your Instagram image.', 'colabsthemes' ); ?>
				</p>
				<p>
					<strong><?php _e( 'Random Instagram:', 'colabsthemes' ); ?></strong>
				</p>
				<p>
					<input type="checkbox" value="true" <?php if(get_post_meta($post->ID,'colabs_random_instagram',true)=='true') echo 'checked="checked"';?> id="colabs_random_instagram"  name="colabs_random_instagram">
					<label><?php _e( 'Random instagram image', 'colabsthemes' ); ?></label>
				</p>
				<p>
					<strong><?php _e( 'Tag Instagram (optional):', 'colabsthemes' ); ?></strong>
				</p>
				<p>	
					<input type="text" value="<?php echo get_post_meta($post->ID,'colabs_tag_instagram',true);?>" id="colabs_tag_instagram"  size="46" name="colabs_tag_instagram"><br/><?php _e( 'Tag (Currently only one tag. Username is ignored.)', 'colabsthemes' ); ?>
				</p>
				<p>
					<strong><?php _e( 'Address/Coordinates Instagram (optional):', 'colabsthemes' ); ?></strong>
				</p>
				<p>	
					<input type="text" value="<?php echo get_post_meta($post->ID,'colabs_address_instagram',true);?>" id="colabs_address_instagram"  size="46" name="colabs_address_instagram"><br/><?php _e( 'Entry Address/Coordinates instagram in here.', 'colabsthemes' ); ?>
				</p>	
			</div>
			
			<div id="flickr_gallery_block" style="display:none">
				<p>
					<strong><?php _e( 'Flickr ID:', 'colabsthemes' ); ?></strong>
				</p>
				<p>	
					<input type="text" value="<?php echo get_post_meta($post->ID,'colabs_username_flickr',true);?>" id="id_flickr" size="46" name="colabs_username_flickr"><br/><?php _e( 'Entry Flickr ID (<a href="http://www.idgettr.com" target="_blank">idGettr</a>) in here.', 'colabsthemes' ); ?>
				</p>
				<p>
					<strong><?php _e( 'Count FLickr:', 'colabsthemes' ); ?></strong>
				</p>
				<p>	
					<input type="text" value="<?php echo get_post_meta($post->ID,'colabs_piccount_flickr',true);?>" id="count_flickr" size="4" name="colabs_piccount_flickr"><br/><?php _e( 'Count of your Flickr image.', 'colabsthemes' ); ?>
				</p>
			</div>
		</div>
...
function themes_meta_save_details_colabs_page_style(){
	...
	if('instagram' == $_POST['meta_style_gallery']){
			update_post_meta($post->ID, "colabs_type_instagram", $_POST['colabs_type_instagram']);
			update_post_meta($post->ID, "colabs_piccount_instagram", $_POST['colabs_piccount_instagram']);
			update_post_meta($post->ID, "colabs_random_instagram", $_POST['colabs_random_instagram']);
			update_post_meta($post->ID, "colabs_tag_instagram", $_POST['colabs_tag_instagram']);
			update_post_meta($post->ID, "colabs_address_instagram", $_POST['colabs_address_instagram']);
	}else if('post' == $_POST['meta_style_gallery']){
			update_post_meta($post->ID, "cat", $_POST['cat']);
	}else if('flickr' == $_POST['meta_style_gallery']){
			update_post_meta($post->ID, "colabs_username_flickr", $_POST['colabs_username_flickr']);
			update_post_meta($post->ID, "colabs_piccount_flickr", $_POST['colabs_piccount_flickr']);
	}
	}		
}
...
if(get_option('colabs_custom_excerpt')=='true'){
...
if( get_option('colabs_footer_credit') != 'true' ) 
	echo '<p class="footer-left">&copy; '. date('Y') .' <a href="http://colorlabsproject.com/themes/'.get_option('colabs_themename').'">'.get_option('colabs_themename').'</a>. All Rights Reserved</p>';
else 
	echo stripslashes( get_option('colabs_footer_credit_txt') ); 
...
$colabs_share_linkedin = get_option('colabs_share_linkedin');
    ...
    if(function_exists('colabs_shortcode_twitter') && $colabs_share_twitter == "true"){
        $tweet_args = array(  'url' => $url,
   							'style' => 'horizontal',
   							'source' => ( get_option('colabs_twitter_username') )? get_option('colabs_twitter_username') : '',
   							'text' => '',
   							'related' => '',
   							'lang' => '',
   							'float' => 'fl'
...
?>
      ...
      <div class="soc-net <?php if($class) echo $class; ?>">
		...
		<a class="rss" href="<?php if(get_option("colabs_feedid") != ''){ echo 'http://feeds.feedburner.com/'.get_option("colabs_feedid");	}else{ bloginfo("rss2_url"); }?>">
		...
		<?php if (get_option("colabs_social_vimeo") != '' ) : ?>
    	<a class="vimeo" href="<?php echo get_option("colabs_social_vimeo");?>"><i class="icon-vimeo"></i></a>
    	<?php endif; ?>
...
add_action('wp_footer','colabs_responsive_menu');
function colabs_responsive_menu(){
  ?>
  <div id="main-menu-slide" class="side-mobilemenu">
    <div class="mm-inner">
      <?php wp_nav_menu( array( 'theme_location' => 'primary', 'container_class' => '', 'menu_class' => 'menu', 'fallback_cb'=>'colabs_nav_fallback') );?>
    </div>
  </div>
  <?php
}
